...
- **Freshness before reporting ticket data**: `mcp__cursus__smart_refresh(organization=...)` — the lightweight path. It diffs Supportal against the local cache on six signals (new IDs, status change, solved-date change, priority escalation, stub records, stale-open) and pulls only what's new/changed, then kicks off background enrichment. It catches exactly the failure modes this report hit before (locally-open tickets that were actually solved; an account whose tickets were never scraped at all). Prefer it over the heavy `check_data_freshness` → `rescrape_customer_tickets` path; only fall back to a full `rescrape_customer_tickets` if `smart_refresh` reports the local copy is badly out of sync (e.g. a large stub count) or the user explicitly asks for a full rescrape.
