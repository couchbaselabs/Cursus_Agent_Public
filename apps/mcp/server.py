...
def _fetch_org_meta(app: Any, organization: str, cookie: str = "") -> dict:
    """
...
""".strip()
    try:
        rows = app.query_supportal_analytics(meta_stmt, cookie)
...
def get_morning_briefing(
    ...
    for org in orgs:
        ...
        _meta = _fetch_org_meta(app, org, cfg.get("cookie", ""))  # best-effort live AE lookup
...
@mcp.tool()
def smart_refresh(organization: str = "", max_new: int = 25) -> str:
    """
    Lightweight freshness check + auto-repair for a customer.
    Diffs Supportal vs local CB to find tickets that are missing locally,
    scrapes ONLY those new tickets (never touches existing data), and starts
    a background enrichment job (embed + score + SFDC correlation) on the new tickets.
    Safe to call whenever the user asks about current status, recent changes, or freshness.
    Returns a summary of what changed.
    Use rescrape_customer_tickets only when the user explicitly asks for a FULL rescrape
    or wants to refresh existing/stale ticket data.

    Args:
        organization: Customer/org name. Defaults to currently scoped customer.
        max_new:      Max new tickets to pull in one call (default 25, max 100). Newest first.
    """
    cfg = _cfg()
    app = _app()
    try:
        ctx = {
            "cookie":         cfg.get("cookie", ""),
            "emb_provider":   cfg.get("emb_provider", ""),
            "emb_model":      cfg.get("emb_model", ""),
            "emb_api_key":    cfg.get("emb_api_key", ""),
            "emb_base_url":   cfg.get("emb_base_url", ""),
            "emb_dims":       cfg.get("emb_dims", 0),
            "embed_parallel": cfg.get("embed_parallel", 1),
            "provider":       cfg.get("score_provider", ""),
            "model":          cfg.get("score_model", ""),
            "api_key":        cfg.get("score_api_key", ""),
            "base_url":       cfg.get("score_base_url", ""),
        }
        result = app._execute_agent_tool(
            "smart_refresh",
            {"organization": organization, "max_new": min(int(max_new), 100)},
            *_cb_tuple(cfg),
            ctx=ctx,
        )
        return json.dumps({"result": result}, default=str)
    except Exception as exc:
        _log_tool_failure("smart_refresh", exc, organization)
        return json.dumps({"error": str(exc)})
...
def _build_health_report_html(org: str, tickets: list[dict], report_date: str, brand: dict, org_meta: dict | None = None, true_total: int = 0,
    ...
    # Include a "No priority" segment so unclassified tickets render as a
    # deliberate gray slice instead of the bar's white background bleeding
    # through (which reads as a broken/white P4 segment). Zero-width
    # segments are skipped entirely.
    _mix_defs = [("P1", "var(--crit)", "P1"), ("P2", "var(--warn)", "P2"),
                 ("P3", "var(--cb)", "P3"), ("P4", "var(--neutral)", "P4"),
                 ("P?", "#C9D0DC", "No priority")]
    mix_bar_html = "".join(
        f'\n      <div class="mix-seg" style="width:{_pct(pri)}%;background:{color};'
        f'{"color:var(--text-2);" if pri == "P?" else ""}" '
        f'title="{lbl}:{priority_counts.get(pri,0)}">{pri if pri != "P?" else ""}</div>'
        for pri, color, lbl in _mix_defs if priority_counts.get(pri, 0)
    )

    mix_legend_html = "".join(
        f'      <div class="mix-legend-item"><span class="mix-swatch" style="background:{color};"></span> {lbl} — {priority_counts.get(pri,0)} ({_pct(pri)}%)</div>\n'
        for pri, color, lbl in [("P1","var(--crit)","P1"),("P2","var(--warn)","P2"),
                                ("P3","var(--cb)","P3"),("P4","var(--neutral)","P4"),
                                ("P?","#C9D0DC","No priority")]
        if pri != "P?" or priority_counts.get("P?", 0)
    )
    ...
    for t in sorted(open_t, key=lambda x: x.get("created") or "")[:8]:
        ...
        # A ticket's annotation may be a single note or a list of notes.
        # An "internal:" prefix marks that note internal-only — it picks up
        # the template's hide-internal toggle (and the customer-copy
        # checkbox) exactly like the renewal badge.
        _raw = (annotations or {}).get(str(tid), "")
        _notes = _raw if isinstance(_raw, list) else ([_raw] if _raw else [])
        _note_html = ""
        for _note in _notes:
            _note = str(_note)
            _internal = _note.startswith("internal:")
            if _internal:
                _note = _note[len("internal:"):].strip()
            _note_cls = ' class="internal-only"' if _internal else ""
            _note_tag = '<strong style="color:var(--warn);">[internal]</strong> ' if _internal else ""
            _note_html += (f'<div{_note_cls} style="grid-column:1/-1;font-size:11px;color:var(--text-2);'
                           f'padding:2px 0 4px 0;border-top:1px dashed var(--border-light);margin-top:4px;">'
                           f'📌 {_note_tag}{_note}</div>')
...
def generate_health_report(
    ...
    org_meta = _fetch_org_meta(app, organization, cfg.get("cookie", ""))
    ...
    try:
        ...
        if annotations:
            try:
                # Values may be a string or a list of strings ("internal:"
                # prefix marks a note internal-only, hidden by the report's
                # customer-copy toggle).
                _ann = {str(k): (v if isinstance(v, list) else str(v))
                        for k, v in json.loads(annotations).items()}
            except Exception:
...
def _sfdc_cb_args() -> tuple:
    cfg = _cfg()
    return (cfg["cb_url"], cfg["bucket"], cfg["username"], cfg["password"],
            cfg["use_tls"], cfg["scope"])


def _sfdc_user_name() -> str:
    """Return the SE display name stored in the active settings profile."""
    try:
        app = _app()
        s = app._load_settings_file()
        active = s.get("__last__", "")
        profile = s.get(active, {}) if active else {}
        return profile.get("sfdc_user_name", "")
    except Exception:
        return ""
...
def get_account_opportunities(organization: str) -> str:
    ...
    try:
        ...
        return query_account_opportunities(organization, *_sfdc_cb_args())
    except Exception as exc:
...
@mcp.tool()
def get_sfdc_field_mapping() -> str:
    """Return the current Salesforce → Couchbase field mapping."""
    try:
        from supportal.sfdc_sync import get_field_mapping_text
        return get_field_mapping_text(*_sfdc_cb_args())
    except Exception as exc:
        return f"get_sfdc_field_mapping error: {exc}"


@mcp.tool()
def update_sfdc_field_mapping(logical_key: str, sfdc_value: str) -> str:
    """Update one entry in the Salesforce field mapping stored in Couchbase."""
    if not logical_key or not sfdc_value:
        return "logical_key and sfdc_value are both required."
    try:
        from supportal.sfdc_sync import update_field_mapping_entry
        return update_field_mapping_entry(logical_key, sfdc_value, *_sfdc_cb_args())
    except Exception as exc:
        return f"update_sfdc_field_mapping error: {exc}"


@mcp.tool()
def sync_sfdc_data() -> str:
    """Trigger a full Salesforce → Couchbase sync (accounts, opps, team assignments) in the background."""
    try:
        import threading as _th
        from supportal.sfdc_sync import sync_all
        _th.Thread(target=sync_all, daemon=True).start()
        return "SFDC sync started in the background. Ask for account data in a few minutes once it completes."
    except Exception as exc:
        return f"sync_sfdc_data error: {exc}"


@mcp.tool()
def get_my_sfdc_accounts() -> str:
    """Get open Salesforce opportunities for the SE whose name is saved in settings (sfdc_user_name)."""
    se_name = _sfdc_user_name().strip()
    if not se_name:
        return (
            "No Salesforce identity configured. "
            "Open the Unified UI → Settings → Salesforce, type your name, click Find, then Save."
        )
    try:
        from supportal.sfdc_sync import query_se_opportunities
        return query_se_opportunities(se_name, *_sfdc_cb_args())
...
@mcp.tool()
def get_account_intelligence(organization: str) -> str:
    """Return a combined intelligence brief for a customer:
    open tickets, SFDC account metadata, licensed products, and open opportunities.
    Use this to detect feature drift (customer using/asking about something not in their license),
    assess customer value (ARR, contract end), and understand SE/AE ownership.
    Trigger this automatically when fleet or account-level questions arise about a specific customer.
    """
    parts = []
    ...
    try:
        cfg = _cfg()
        app = _app()
        rows = app.tool_query_tickets(
            {"organization": organization},
            *_cb_tuple(cfg),
            limit=10,
        )
        if rows:
            parts.append(f"### Open Tickets ({len(rows)})")
            for r in rows[:10]:
                parts.append(f"- #{r.get('ticket_id','')} [{r.get('priority','')}] {r.get('subject','')[:80]}")
        else:
            parts.append("### Open Tickets\nNone.")
    except Exception as e:
        parts.append(f"### Open Tickets\nError: {e}")

    # --- SFDC context ---
    try:
        from supportal.sfdc_sync import get_account_sfdc_context
        ...
        if ctx:
            ...
            parts.append(f"- **Active PS Projects**: {ctx.get('active_ps_projects') or 0}")

            products = ctx.get("licensed_products") or []
            if products:
                parts.append(f"\n### Licensed Products ({len(products)})")
                for p in products:
                    parts.append(f"- {p}")
            else:
                parts.append("\n### Licensed Products\nNone on record.")

            opps = ctx.get("open_opportunities") or []
            if opps:
                parts.append(f"\n### Open Opportunities ({len(opps)})")
                for o in opps:
                    parts.append(f"- **{o.get('opp_name','')}** | Stage: {o.get('stage','')} | Close: {o.get('close_date','')[:10]} | ARR: ${o.get('arr') or 0:,.0f}")
                    sup = o.get("supporting_se_name")
                    if sup:
                        parts.append(f"  Supporting SE: {sup}")
        else:
            parts.append("\n### SFDC Account\nNo SFDC data synced for this org.")
    except Exception as e:
        parts.append(f"\n### SFDC Account\nError: {e}")

    return "\n".join(parts) if parts else f"No data found for '{organization}'."
