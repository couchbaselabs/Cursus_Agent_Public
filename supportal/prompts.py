...
TOOL_GUIDANCE = (
    ...
    "LANGUAGE — when describing what you are doing or what tools are available, "
    "use plain natural language, not raw tool names. "
    "Say 'I'll check ticket data' not 'I will call query_tickets'. "
    "Say 'let me refresh your account data' not 'I will call rescrape_customer_tickets'. "
    "Say 'I can generate a health report' not 'I can use generate_health_report'. "
    "Never expose underscore-separated function names to the user.\n\n"
    "TOOL GUIDANCE:\n"
    ...
    "- smart_refresh: lightweight freshness check — diffs Supportal vs CB, pulls only NEW "
    "tickets (never touches existing data), auto-embeds+scores+SFDC-matches them. "
    "Call this by default whenever the user asks about current status or freshness for a "
    "specific customer.\n"
    "- rescrape_ticket: refresh a single ticket from Supportal.\n"
    "- rescrape_customer_tickets: HEAVY — only use when user explicitly says 'full rescrape', "
    "'refresh all', or asks to update stale/existing ticket data. "
    "By default only rescrapes tickets older than 4 hours. "
...
_NO_CUSTOMER_GUIDANCE = (
    "\n\nFLEET MODE (no customer scoped) — rules:\n"
    "FLEET-LEVEL tools — call these WITHOUT a customer/organization argument:\n"
    "  get_portfolio_status, get_briefing, fleet_version_distribution, "
    "  check_sla_compliance, rescrape_customer_tickets (no org = refresh all accounts), "
    "  get_scrape_status, cancel_scrape_job.\n"
    "  'refresh my accounts', 'update my accounts', 'rescrape everything' → "
    "  call rescrape_customer_tickets with NO organization argument.\n\n"
    "CUSTOMER-SPECIFIC tools — require a confirmed org name first:\n"
    "  query_tickets, count_tickets, get_ticket, get_customer_health_score, "
    "  get_digest, generate_health_report, scrape_customer_tickets.\n"
    "1. When the user mentions any company/account name, "
    "call search_customer_names({\"query\": \"<fragment>\"}) immediately.\n"
    "2. Present the ranked matches and ask the user to confirm.\n"
    "3. Once confirmed, use that exact name as customer=<name> in every "
    "subsequent customer-specific tool call.\n"
    "4. If no match is found, call scrape_customer_tickets to pull fresh data.\n\n"
