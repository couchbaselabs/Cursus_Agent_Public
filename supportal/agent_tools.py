...
_AGENT_TOOLS: list[dict] = [
    ...
    # ── v2.7.64: smart_refresh ────────────────────────────────────────────────
    {
        "type": "function",
        "function": {
            "name": "smart_refresh",
            "description": (
                "Lightweight multi-signal freshness diff + auto-repair for a customer. "
                "Compares Supportal listing against CB on six signals: "
                "(1) new ticket IDs missing from CB, "
                "(2) status changed (e.g. open→solved), "
                "(3) solved date changed (closed/reopened), "
                "(4) priority escalated (e.g. P3→P1), "
                "(5) stub records (detail page was never fetched), "
                "(6) stale-open tickets (open/pending not scraped in >4h — proxy for updated_at, "
                "which the Supportal listing API does not expose). "
                "Also reports enrichment gaps: tickets missing score, embedding, or SFDC correlation. "
                "Scrapes ONLY the changed/new tickets and starts a background enrichment job "
                "(embed → score → SFDC) on them. Safe to call whenever the user asks about "
                "current status, recent changes, or freshness. "
            ...
            "parameters": {
                ...
                "properties": {
                    "organization": {
                        "type": "string",
                        "description": "Customer/org name. Defaults to currently scoped customer.",
                    },
                    "max_new": {
                        "type": "integer",
                        "description": "Max tickets to process in one call (default 25, max 100). Newest first.",
                    },
                },
                "required": [],
            },
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Generate a rich branded HTML health report for a customer and save it as an asset. "
                "Includes health score, open ticket table, SLA compliance, cluster topology, trend charts, "
                "and account contacts. Brand colors/logo are applied automatically if a brand kit exists. "
                "The saved asset appears in the Reports tab and is print/PDF ready. "
                "Call when the user asks for a 'report', 'health report', 'customer report', 'status report', "
                "'generate a report', or 'create a report' for a specific customer."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "organization": {"type": "string", "description": "Customer/org name."},
                    "ae_name":      {"type": "string", "description": "Account Executive name (optional; auto-filled from org record)."},
                    "tse_name":     {"type": "string", "description": "TSE name (optional; auto-filled from org record)."},
                    "pse_name":     {"type": "string", "description": "PSE/SE name (optional)."},
                    "date_from":    {"type": "string", "description": "ISO date YYYY-MM-DD — analyze only tickets on/after this date."},
                    "date_to":      {"type": "string", "description": "ISO date YYYY-MM-DD — analyze only tickets on/before this date."},
                    "annotations":  {"type": "string", "description": "JSON object mapping ticket_id to a note shown under that ticket."},
                },
                "required": ["organization"],
            },
        },
    ...
    # ── SFDC tools ───────────────────────────────────────────────────────────
    {
        ...
        "function": {
            ...
            "description": (
                "Get all open Salesforce opportunities for a specific account. "
                "Returns opp name, stage, close date, SE, ARR, renewal ARR, and any blocking CBSEs. "
                "Use when asked about deals, renewals, opportunities, or pipeline for an account."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "organization": {"type": "string", "description": "Account / org name to look up."},
                },
                "required": ["organization"],
            },
        },
    },
    {
        "type": "function",
        "function": {
            "name": "get_se_opportunities",
            "description": (
                "Get all open Salesforce opportunities across all accounts where a given person "
                "is the primary SE or supporting SE. "
                "Use when asked 'what opps am I on?', 'show Austin\\'s pipeline', or any SE-scoped opportunity question. "
                "If no se_name is given, use the current user's name."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "se_name": {"type": "string", "description": "SE name to filter by (partial match)."},
                },
                "required": ["se_name"],
            },
        },
    },
    {
        "type": "function",
        "function": {
            "name": "list_sfdc_accounts",
            "description": (
                "List Salesforce accounts with team assignments (AE, SE, CSM) and contract dates. "
                "Optionally filter to accounts where a specific person is AE, SE, or CSM. "
                "Use for 'what accounts am I on?', 'show Guna\\'s accounts', or browsing the book of business."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "se_name": {"type": "string", "description": "Filter to accounts where this person is AE, SE, or CSM. Leave blank for all accounts."},
                },
                "required": [],
            },
        },
    },
    {
        "type": "function",
        "function": {
            "name": "get_sfdc_field_mapping",
            "description": (
                "Show the current Salesforce field mapping — the logical keys and the SFDC API field names "
                "they map to. Use when the user wants to see or audit the field configuration."
            ),
            "parameters": {"type": "object", "properties": {}, "required": []},
        },
    },
    {
        "type": "function",
        "function": {
            "name": "update_sfdc_field_mapping",
            "description": (
                "Update a single entry in the Salesforce field mapping. "
                "Use when a SFDC field has been renamed and the sync needs to point to the new API name. "
                "Takes effect on the next sync run."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "logical_key": {"type": "string", "description": "The logical key to update (e.g. 'opp_arr')."},
                    "sfdc_value":  {"type": "string", "description": "The new SFDC API field name or role string."},
                },
                "required": ["logical_key", "sfdc_value"],
            },
        },
    },
    {
        ...
        "function": {
            "name": "sync_sfdc_data",
            "description": (
                "Trigger a full Salesforce → Couchbase sync. Harvests accounts, team assignments, "
                "opportunities, products, and PS projects. Runs in the background. "
                "Use when asked to 'sync SFDC', 'refresh account data', or 'update Salesforce data'."
            ),
            "parameters": {"type": "object", "properties": {}, "required": []},
        },
    },
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Return a combined intelligence brief for a customer: open support tickets, "
                "SFDC account metadata (ARR, contract end, AE/SE/CSM), licensed Couchbase products, "
                "and open Salesforce opportunities. Use this to detect feature drift (customer using "
                "or asking about something not in their license), assess customer value, understand "
                "team ownership, and identify champions. Trigger automatically when a fleet or "
                "account-level question arises about a specific customer."
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
def call_llm_with_tools(
    ...
    tool_choice: str = "auto",  # "auto" | "required" — passed to the API on round 0
    ...
    if provider in _openai_compat_providers:
        ...
        _ant_tool_choice = {"type": "any"} if tool_choice == "required" else {"type": "auto"}
        for _round in range(max_rounds):
            _check_cancel()
...
def _query_fleet_tickets(
    ...
    """
    _status_clause = {
        "open":   "AND LOWER(t.status) IN ['open','new']",
        "pending": "AND LOWER(t.status) = 'pending'",
...
def _fleet_version_distribution(
    ...
    sql = (
        ...
        f"COUNT(DISTINCT s.cluster_uuid) AS cluster_count, "
        f"COUNT(DISTINCT s.organization) AS org_count "
        f"FROM `{bucket}`.`{scope}`.`{snap_collection}` s "
        f"WHERE s.cb_version IS NOT NULL AND s.cb_version != '' "
        f"GROUP BY s.cb_version ORDER BY cluster_count DESC LIMIT 30"
    )
    return _fleet_query(cb_url, bucket, username, password, use_tls, scope, sql)


def _fleet_ticket_freshness(
    cb_url, bucket, username, password, use_tls, scope, collection,
):
    """Per-org: most recent ticket activity (updated > last_comment_at > created) + scrape time."""
    sql = (
        f"SELECT t.organization, "
        f"MAX(IFNULL(t.updated, IFNULL(t.last_comment_at, t.created))) AS last_activity, "
        f"MAX(t.last_scraped_at) AS last_scraped, "
        f"COUNT(*) AS ticket_count "
        f"FROM `{bucket}`.`{scope}`.`{collection}` t "
        f"WHERE t.organization IS NOT NULL "
        f"GROUP BY t.organization "
